        ...
        if (r->script) {
                ...
                if (STR_EQ(r->name, "global")) {
                        if (save && n->original->script == NULL)
                                n->original->script = n->insert_script_count > 0
                                                    ? g_strdup(n->insert_scripts[0])
                                                    : g_strdup(r->script);

                        n->insert_scripts = g_renew(char *, n->insert_scripts, n->insert_script_count + 2);
                        n->insert_scripts[n->insert_script_count] = g_strdup(r->script);
                        n->insert_scripts[n->insert_script_count + 1] = NULL;
